	...
	<div class="main">
		...
		<div class="content">
			<div class="grid wide">
				<div class="row">
					<div class="col l-12">
						<input type="button" value="Thêm loại sản phẩm" onclick="window.location.href='showFormForAdd'; return false;" class="btn btn--add"
							...
							<tbody>
								...
								<c:forEach var="tempCategory" items="${categories }">
									<c:url var="updateLink"
										value="/manager/category/showFormForUpdate">
										<c:param name="categoryId" value="${tempCategory.id}" />
									</c:url>
									<c:url var="deleteLink" value="/manager/category/delete">
										<c:param name="categoryId" value="${tempCategory.id}" />
									</c:url>
									<tr>
										<td><%=id++%></td>
										...
										<td>
											<a href="${updateLink }">Update</a>|<a href="${deleteLink }" onclick="if (!(confirm('Are you sure you want to delete this category?'))) return false">Delete</a>
										</td>
									</tr>
								</c:forEach>
							</tbody>
